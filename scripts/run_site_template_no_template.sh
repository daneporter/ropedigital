...
ssh $username@$ip << EOF
cd /var/opt/ 
wget -q https://raw.githubusercontent.com/daneporter/ropedigital/master/playbooks/starter_site_playbook_no_template.sh -O starter_site_playbook_no_template.sh
chmod 700 starter_site_playbook_no_template.sh
EOF
...
ssh $username@$ip << EOF
screen -d -m /var/opt/starter_site_playbook_no_template.sh $app_name $usr_name $template_id $email $sub_domain
EOF
